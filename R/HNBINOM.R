...
#' @param prob probability of success in each trial. 0 < prob \code{\leq} 1
...
dhnbinom <-
function(x, theta = 0.5, size = 1, prob = 1, log = FALSE) {
  zindex <- x == 0
  x[zindex] <- theta
  x[!zindex] <- (1 - theta) * dnbinom(x[!zindex], size = size, prob) / (1 - dnbinom(0, size = size, prob))
