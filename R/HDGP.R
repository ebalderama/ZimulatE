...
#' generalized Pareto distribution with parameters \code{theta}, \code{mu}, \code{lambda}, and \code{alpha}.
#' @param x,q vector of quantiles
#' @param p vector of probabilities
#' @param n number of observations
#' @param theta zero generating parameter (probability of zeros)
#' @param mu location parameter
#' @param lambda (non-negative) scale parameter
...
function(x, theta = 0.5, mu=0, lambda=1, alpha=1, log = FALSE) {
  zindex <- x == 0
  x[zindex] <- theta
  x[!zindex] <- (1 - theta) * ddgp(x[!zindex], mu, lambda, alpha)
  x
}
...
#GPD:(continuous)
rhdgp <- function(n, theta = 0.5, mu = 0, lambda = 1, alpha = 1){
  
  zero <- rbinom(n, 1, theta)
  sum_zero <- sum(zero == 1)
  sum_non_zero <- n - sum_zero
  z_trun <- rdgp(sum_non_zero, mu = 0.5, lambda, alpha)
