...
ddhnbinom <-
  function(x, theta = 0.5, theta_1 = 0.1, size = 0.9, mu = 0.9, scale = 1, shape = 1, threshold = 3, log = FALSE) {
    ...
    tt[x == 0] <- theta
    tt[x >= 1 & x < threshold] <-
      (1 - theta) * (1 - theta_1) * dnbinom(x[x >= 1 & x < threshold], size = size, mu = mu) / (pnbinom(threshold - 1, size = size, mu = mu) - pnbinom(0, size = size, mu = mu))
...
rdhnbinom <- function(n, theta=0.5,theta_1=0.1, size = 0.9, mu_1 = 0.9, sigma = 1, xi = 1, threshold=3){
  ...
  y2 <- rdgp(n_last, mu=threshold, sigma, xi)
